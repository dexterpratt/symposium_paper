---
name: rdaneel-review-afternoon
description: rdaneel scheduled-review mode — afternoon digest (14:15 local, last in afternoon batch)
---
...
6. **Publish the digest.** Create a `ndex-message-type: report` network named:
   ...
   `ndexagent rdaneel daily digest YYYY-MM-DD afternoon`
   ...
   - **Afternoon-specific**: cross-reference today's morning digest if one exists (search `ndexagent rdaneel daily digest YYYY-MM-DD morning`) and note new developments since morning under a `since_morning` attribute on the root node — this is the principal value-add of the afternoon run.
