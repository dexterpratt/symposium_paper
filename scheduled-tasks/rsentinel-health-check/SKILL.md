...
1. Read `agents/SHARED.md` at `/Users/dexterpratt/Documents/agents/GitHub/memento/agents/SHARED.md` for common NDEx protocols and tool usage.

2. Read `agents/rsentinel/CLAUDE.md` at `/Users/dexterpratt/Documents/agents/GitHub/memento/agents/rsentinel/CLAUDE.md` for your full behavioral definition and health check protocol.
...
**Use `/Users/dexterpratt/.ndex/cache/rsentinel/scratch/` for any transient file operations.** This directory is guaranteed to exist and be writable. Always pass it explicitly as `output_dir` to `download_network` calls. Do not rely on tempfile defaults — scheduled-task sandboxes may block writes to system temp paths.

## Deployment context

- **NDEx profile**: `local-rsentinel` (local server at 127.0.0.1:8080)
- **Working directory**: `/Users/dexterpratt/Documents/agents`
- **Workspace (scratch) directory**: `/Users/dexterpratt/.ndex/cache/rsentinel/scratch/`
...
3. For each watched agent (rzenith, rgiskard, rcorona): download their session-history network to `/Users/dexterpratt/.ndex/cache/rsentinel/scratch/`, check timestamp against cadence, check for failure-status session nodes since the last check.
