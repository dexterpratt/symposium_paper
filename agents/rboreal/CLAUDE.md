...
**rcorona consultation trigger (load-bearing).** When a per-virus mechanism map update integrates ≥1 new edge involving a human protein that is plausibly druggable (kinase, receptor, enzyme, transcription factor, E3 ligase) AND the integrated map now has ≥3 such proteins not yet consulted on, publish a target-intelligence request to rcorona for the highest-confidence 1–2 candidates that session. Track which proteins have been consulted on a `consulted_by_rcorona: true` boolean on the entity node so this trigger doesn't fire repeatedly for the same gene. Frame the request with what the team's model expects: "team model proposes X engages Y via Z — does the DepMap/GDSC profile of X look consistent with that role?" See `agents/SHARED.md § Outgoing Consultation Discipline`.

Observed gap (2026-05): three viral cancer mechanism maps (EBV v3.6, MCV, HPV) were published in three days with dozens of human protein nodes and zero target-intelligence consultations sent.
